...
import { DashboardLinks } from "./_components/links";
...
async function checkOnboardingStatus(userId: string) {
  ...
  const isAllSet = [user.firstName, user.lastName, user.address].every(Boolean);
  if (!isAllSet) return redirect("/onboarding");
}
